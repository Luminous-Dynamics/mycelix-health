...
def self_test():
    ...
    with tempfile.TemporaryDirectory() as raw:
        ...
        common=["--release-bundle",str(bundle),"--compatibility-report",str(compat),"--supply-chain-report",str(supply),"--reproducibility-report",str(repro),"--reproducibility-provenance",str(repro_prov),"--attestation-subjects",str(subjects),"--attestation-report",str(attest),"--suite-root",str(suite)]
        preflight=root/"preflight.json"; run("python3","scripts/preflight-clinical-promotion.py",*common,"--output",str(preflight))
        if json.loads(preflight.read_text()).get("status")!="verified": fail("valid promotion preflight was not verified")
        rehearsal=root/"rehearsal.json"; run("python3","scripts/rehearse-clinical-promotion.py",*common,"--output",str(rehearsal))
        rehearsal_value=json.loads(rehearsal.read_text())
        if rehearsal_value.get("status")!="verified" or rehearsal_value.get("stage_results",[])[-1].get("status")!="verified": fail("valid promotion rehearsal was not verified")
        out=root/"out"; cmd=["python3","scripts/promote-clinical-release.py",*common,"--output-dir",str(out)]; run(*cmd)
        ...
        bad=json.loads(supply.read_text()); bad["materials"]["sbom"]["sha256"]="0"*64; badpath=root/"bad.json"; write(badpath,bad); refused=root/"refused"; refusal_report=root/"refusal.json"; badcmd=cmd.copy(); badcmd[badcmd.index(str(supply))]=str(badpath); badcmd[-1]=str(refused); badcmd.extend(["--refusal-output",str(refusal_report)]); run(*badcmd,expected=1)
        if refused.exists(): fail("refused promotion created partial output")
        refusal_value=json.loads(refusal_report.read_text())
        if refusal_value.get("reason",{}).get("code")!="CURRENT_MATERIAL_DIGEST_MISMATCH" or refusal_value.get("reason",{}).get("stage")!="supply_chain": fail("structured refusal classification drifted")
        if refusal_value.get("promotion_output_created") is not False or refusal_report.stat().st_mode & 0o777 != 0o600: fail("refusal evidence is not create-only owner-readable")
        bad_common=common.copy(); bad_common[bad_common.index(str(supply))]=str(badpath)
        bad_rehearsal=root/"bad-rehearsal.json"; run("python3","scripts/rehearse-clinical-promotion.py",*bad_common,"--output",str(bad_rehearsal),expected=3)
        bad_value=json.loads(bad_rehearsal.read_text())
        if bad_value.get("reason_graph",{}).get("root_cause_stages")!=["supply_chain"]: fail("rehearsal did not isolate the supply-chain root cause")
        explanation=root/"explanation.json"; run("python3","scripts/explain-clinical-promotion.py","--report",str(bad_rehearsal),"--report",str(refusal_report),"--output",str(explanation))
        explained=json.loads(explanation.read_text())
        if explained.get("status")!="action-required" or explained.get("root_causes",[])[0].get("owner")!="supply-chain": fail("refusal explanation did not prioritize supply-chain remediation")
        repeated=run(*badcmd,expected=1)
        if "Traceback" in (repeated.stdout+repeated.stderr) or "clinical refusal evidence was not written" not in repeated.stdout: fail("existing refusal output was not handled safely")
        invalid_output=root/"invalid-output"; invalid_refusal=invalid_output/"refusal.json"; invalid_cmd=cmd.copy(); invalid_cmd[-1]=str(invalid_output); invalid_cmd.extend(["--refusal-output",str(invalid_refusal)]); invalid=run(*invalid_cmd,expected=1)
        if invalid_output.exists() or "Traceback" in (invalid.stdout+invalid.stderr): fail("invalid refusal placement created partial output or traceback")
...
def main():
    ...
    for rel in ("scripts/build-clinical-supply-chain-report.py","scripts/verify-release-attestations.py","scripts/clinical-promotion-diagnostics.py","scripts/preflight-clinical-promotion.py","scripts/rehearse-clinical-promotion.py","scripts/promote-clinical-release.py","scripts/explain-clinical-promotion.py","scripts/check-clinical-promotion.py"):
