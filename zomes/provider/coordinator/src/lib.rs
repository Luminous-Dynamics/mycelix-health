...
pub fn update_provider(input: UpdateProviderInput) -> ExternResult<Record> {
    ...
    let original_record = get(input.original_hash.clone(), GetOptions::default())?.ok_or(
        wasm_error!(WasmErrorInner::Guest("Provider not found".to_string())),
    )?;
...
pub fn add_license(license: License) -> ExternResult<Record> {
    let caller = agent_info()?.agent_initial_pubkey;
    let provider_record = get(license.provider_hash.clone(), GetOptions::default())?.ok_or(
        wasm_error!(WasmErrorInner::Guest("Provider not found".to_string())),
    )?;
...
pub fn add_board_certification(cert: BoardCertification) -> ExternResult<Record> {
    let caller = agent_info()?.agent_initial_pubkey;
    let provider_record = get(cert.provider_hash.clone(), GetOptions::default())?.ok_or(
        wasm_error!(WasmErrorInner::Guest("Provider not found".to_string())),
    )?;
    if provider_record.action().author() != &caller {
        return Err(wasm_error!(WasmErrorInner::Guest(
...
pub fn create_provider_patient_relationship(
    ...
    let caller = agent_info()?.agent_initial_pubkey;
    let provider_record = get(relationship.provider_hash.clone(), GetOptions::default())?.ok_or(
        wasm_error!(WasmErrorInner::Guest("Provider not found".to_string())),
    )?;
    let provider_author = provider_record.action().author().clone();

    let patient_record = get(relationship.patient_hash.clone(), GetOptions::default())?.ok_or(
        wasm_error!(WasmErrorInner::Guest("Patient not found".to_string())),
    )?;
...
mod tests {
    ...
    fn test_serde_roundtrip_credential_verification_result() {
        ...
        let decoded: CredentialVerificationResult =
