...
pub fn record_data_usage(usage: DataUsage) -> ExternResult<Record> {
    let contribution_record = get(usage.contribution_hash.clone(), GetOptions::default())?.ok_or(
        wasm_error!(WasmErrorInner::Guest("Contribution not found".to_string())),
    )?;
    let contribution: DataContribution = contribution_record
        ...
        .map_err(|e| wasm_error!(WasmErrorInner::Guest(e.to_string())))?
        .ok_or(wasm_error!(WasmErrorInner::Guest(
            "Invalid contribution entry".to_string()
        )))?;
...
pub fn get_contribution_usages(contribution_hash: ActionHash) -> ExternResult<Vec<Record>> {
    let contribution_record = get(contribution_hash.clone(), GetOptions::default())?.ok_or(
        wasm_error!(WasmErrorInner::Guest("Contribution not found".to_string())),
    )?;
    let contribution: DataContribution = contribution_record
        .entry()
...
pub fn get_project_contributions(project_hash: ActionHash) -> ExternResult<Vec<Record>> {
    ...
    let project_record = get(project_hash.clone(), GetOptions::default())?.ok_or(wasm_error!(
