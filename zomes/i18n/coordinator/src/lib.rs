...
pub fn get_supported_locales(_: ()) -> ExternResult<Vec<Record>> {
    ...
    let links = get_links(
        GetLinksInputBuilder::try_new(anchor, LinkTypes::AllLocales)?.build(),
    )?;
...
pub fn get_source_strings(namespace: String) -> ExternResult<Vec<Record>> {
    ...
    let links = get_links(
        GetLinksInputBuilder::try_new(anchor, LinkTypes::SourcesByNamespace)?.build(),
    )?;
...
pub fn get_translations_for_locale(locale_tag: String) -> ExternResult<Vec<Record>> {
    ...
    let links = get_links(
        GetLinksInputBuilder::try_new(anchor, LinkTypes::TranslationsByLocale)?.build(),
    )?;
...
pub fn get_translation(input: GetTranslationInput) -> ExternResult<Option<Record>> {
    ...
    let links = get_links(
        GetLinksInputBuilder::try_new(key_anchor, LinkTypes::TranslationsByKey)?.build(),
    )?;
...
#[hdk_extern]
pub fn get_medical_term(terminology: String, code: String, locale: String) -> ExternResult<Option<Record>> {
    let anchor = anchor_hash(&format!("medterm_{}_{}", terminology, code))?;
    let links = get_links(
        GetLinksInputBuilder::try_new(anchor, LinkTypes::MedicalTermsByCode)?.build(),
    )?;
...
pub fn get_my_preferences(_: ()) -> ExternResult<Option<Record>> {
    ...
    let links = get_links(
        GetLinksInputBuilder::try_new(user_hash, LinkTypes::UserToPreferences)?.build(),
    )?;
...
#[hdk_extern]
pub fn add_to_translation_memory(
    source_text: String,
    ...
    // Link by locale pair
    let pair_anchor = anchor_hash(&format!("tm_{}_to_{}", source_locale, target_locale))?;
    create_link(
        pair_anchor,
        action_hash.clone(),
...
pub fn add_glossary_entry(
    ...
    // Link by domain
    let domain_anchor = anchor_hash(&format!("glossary_{}", domain))?;
    create_link(
        domain_anchor,
        action_hash.clone(),
...
#[hdk_extern]
pub fn get_glossary_entries(domain: String) -> ExternResult<Vec<Record>> {
    let anchor = anchor_hash(&format!("glossary_{}", domain))?;
    let links = get_links(
        GetLinksInputBuilder::try_new(anchor, LinkTypes::GlossaryByDomain)?.build(),
    )?;
