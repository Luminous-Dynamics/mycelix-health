...
fn proof_type_to_category(proof_type: &HealthProofType) -> DataCategory {
    match proof_type {
        HealthProofType::VaccinationStatus => DataCategory::Immunizations,
        HealthProofType::InsuranceQualification => DataCategory::All,
        HealthProofType::AgeVerification => DataCategory::Demographics,
        HealthProofType::ConditionPresence | HealthProofType::ConditionAbsence => DataCategory::Diagnoses,
        HealthProofType::EmploymentPhysical => DataCategory::VitalSigns,
        HealthProofType::SubstanceScreening => DataCategory::LabResults,
        HealthProofType::LabThreshold => DataCategory::LabResults,
        HealthProofType::AllergyStatus => DataCategory::Allergies,
        HealthProofType::MedicationStatus => DataCategory::Medications,
        HealthProofType::OrganDonorCompatibility => DataCategory::All,
        HealthProofType::ClinicalTrialEligibility => DataCategory::All,
        HealthProofType::PhysicalCapability => DataCategory::VitalSigns,
        HealthProofType::MentalHealthClearance => DataCategory::MentalHealth,
        HealthProofType::TravelHealthClearance => DataCategory::Immunizations,
        HealthProofType::GeneralHealth => DataCategory::All,
        HealthProofType::Custom(_) => DataCategory::All,
    }
}

fn ensure_patient_self(patient_hash: &ActionHash) -> ExternResult<()> {
    let caller = agent_info()?.agent_initial_pubkey;
    let patient_record = get(patient_hash.clone(), GetOptions::default())?
        .ok_or(wasm_error!(WasmErrorInner::Guest("Patient not found".to_string())))?;
    if patient_record.action().author() != &caller {
        return Err(wasm_error!(WasmErrorInner::Guest(
            "Only the patient can perform this action".to_string()
        )));
    }
    Ok(())
}
...
pub fn generate_health_proof(proof: HealthProof) -> ExternResult<Record> {
    ...
    let data_category = proof_type_to_category(&proof.proof_type);
...
pub fn get_patient_proofs(patient_hash: ActionHash) -> ExternResult<Vec<Record>> {
    ...
    let links = get_links(
        LinkQuery::try_new(patient_hash.clone(), LinkTypes::PatientToProofs)?,
        GetStrategy::default(),
    )?;
...
pub fn revoke_proof(input: RevokeProofInput) -> ExternResult<Record> {
    ...
    let patient_hash = proof.patient_hash.clone();
    ensure_patient_self(&patient_hash)?;

    let auth = require_authorization(
        patient_hash.clone(),
        proof_type_to_category(&proof.proof_type),
        Permission::Amend,
        false,
    )?;
...
pub fn get_pending_requests(patient_hash: ActionHash) -> ExternResult<Vec<Record>> {
    ...
    let links = get_links(
        LinkQuery::try_new(patient_hash.clone(), LinkTypes::PatientToRequests)?,
        GetStrategy::default(),
    )?;
...
pub fn respond_to_request(input: RespondToRequestInput) -> ExternResult<Record> {
    ...
    let patient_hash = request.patient_hash.clone();
    ensure_patient_self(&patient_hash)?;
    let auth = require_authorization(
        patient_hash.clone(),
        DataCategory::All,
...
pub fn get_proof_verifications(proof_hash: ActionHash) -> ExternResult<Vec<Record>> {
    let proof_record = get(proof_hash.clone(), GetOptions::default())?
        .ok_or(wasm_error!(WasmErrorInner::Guest("Proof not found".to_string())))?;
    let proof: HealthProof = proof_record
        .entry()
        .to_app_option()
...
pub fn set_proof_parameters(params: ProofParameters) -> ExternResult<Record> {
    ...
    ensure_patient_self(&params.patient_hash)?;
    let auth = require_authorization(
...
pub fn get_proof_parameters(patient_hash: ActionHash) -> ExternResult<Option<Record>> {
    ...
    let links = get_links(
        LinkQuery::try_new(patient_hash.clone(), LinkTypes::PatientToParameters)?,
        GetStrategy::default(),
    )?;
...
pub fn is_proof_type_allowed(input: CheckProofTypeInput) -> ExternResult<bool> {
    ensure_patient_self(&input.patient_hash)?;
