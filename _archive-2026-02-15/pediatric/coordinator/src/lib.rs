...
pub fn get_growth_history(patient_hash: ActionHash) -> ExternResult<Vec<GrowthMeasurement>> {
    ...
    let links = get_links(
        LinkQuery::try_new(patient_hash.clone(), LinkTypes::PatientToGrowth)?,
        GetStrategy::default(),
    )?;
...
pub fn get_immunization_history(patient_hash: ActionHash) -> ExternResult<Vec<ImmunizationRecord>> {
    ...
    let links = get_links(
        LinkQuery::try_new(patient_hash.clone(), LinkTypes::PatientToImmunizations)?,
        GetStrategy::default(),
    )?;
...
pub fn get_well_child_visits(patient_hash: ActionHash) -> ExternResult<Vec<WellChildVisitRecord>> {
    ...
    let links = get_links(
        LinkQuery::try_new(patient_hash.clone(), LinkTypes::PatientToWellChildVisits)?,
        GetStrategy::default(),
    )?;
