...
pub fn upsert_matching_profile(profile: MatchingProfile) -> ExternResult<Record> {
    ...
    // Link to patient
    let patient_hash = profile.patient_hash.clone();
    create_link(
        patient_hash.clone(),
        profile_hash,
...
pub fn get_patient_profile(patient_hash: ActionHash) -> ExternResult<Option<Record>> {
    ...
    let records = get_linked_records(patient_hash.clone(), LinkTypes::PatientToProfile)?;
    // Return most recent
...
pub fn update_preferences(preferences: PatientPreferences) -> ExternResult<Record> {
    let auth = require_authorization(
        preferences.patient_hash.clone(),
        DataCategory::All,
        Permission::Write,
        false,
    )?;
    ...
    // Link to patient
    let patient_hash = preferences.patient_hash.clone();
    create_link(
        patient_hash.clone(),
        pref_hash,
...
pub fn get_patient_preferences(patient_hash: ActionHash) -> ExternResult<Option<Record>> {
    ...
    let records = get_linked_records(patient_hash.clone(), LinkTypes::PatientToPreferences)?;
    let latest = records.into_iter().last();
...
pub fn get_patient_matches(patient_hash: ActionHash) -> ExternResult<Vec<Record>> {
    ...
    let records = get_linked_records(patient_hash.clone(), LinkTypes::PatientToMatches)?;
...
pub fn send_trial_notification(notification: TrialNotification) -> ExternResult<Record> {
    ...
    // Link to patient
    let patient_hash = notification.patient_hash.clone();
    create_link(
        patient_hash.clone(),
        notif_hash,
...
pub fn get_patient_notifications(patient_hash: ActionHash) -> ExternResult<Vec<Record>> {
    ...
    let records = get_linked_records(patient_hash.clone(), LinkTypes::PatientToNotifications)?;
