...
fn require_device_owner(device_hash: &ActionHash) -> ExternResult<RegisteredDevice> {
    ...
    let device: RegisteredDevice = record
        .entry()
        .to_app_option()
        .map_err(|e| wasm_error!(WasmErrorInner::Guest(e.to_string())))?
        .ok_or(wasm_error!(WasmErrorInner::Guest("Invalid device entry".to_string())))?;

    if device.owner_agent != caller {
        return Err(wasm_error!(WasmErrorInner::Guest(
            "Unauthorized: device is private to owner".to_string()
        )));
    }
...
pub fn update_device_status(input: UpdateDeviceStatusInput) -> ExternResult<ActionHash> {
    let mut device = require_device_owner(&input.device_hash)?;
...
pub fn heartbeat(device_hash: ActionHash) -> ExternResult<ActionHash> {
    let mut device = require_device_owner(&device_hash)?;
...
pub fn create_checkpoint(input: CreateCheckpointInput) -> ExternResult<ActionHash> {
    ...
    let _device = require_device_owner(&input.device_hash)?;
...
pub fn get_latest_checkpoint(device_hash: ActionHash) -> ExternResult<Option<Record>> {
    let _device = require_device_owner(&device_hash)?;
...
pub fn mark_sync_complete(input: MarkSyncCompleteInput) -> ExternResult<ActionHash> {
    ...
    let _device = require_device_owner(&entry.device_hash)?;
...
pub fn mark_notification_sent(notification_hash: ActionHash) -> ExternResult<ActionHash> {
    ...
    let _device = require_device_owner(&notif.device_hash)?;
...
pub fn get_emergency_snapshot(patient_hash: ActionHash) -> ExternResult<Option<Record>> {
    ...
    let links = get_links(
        LinkQuery::try_new(patient_hash.clone(), LinkTypes::PatientToEmergencySnapshot)?,
        GetStrategy::default(),
    )?;
    ...
    let record = latest.map(|(_, record)| record);
    if record.is_some() {
        log_data_access(
            patient_hash,
            vec![DataCategory::All],
