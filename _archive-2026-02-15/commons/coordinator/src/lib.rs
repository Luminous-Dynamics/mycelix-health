...
pub fn get_pool_contributions(pool_hash: ActionHash) -> ExternResult<Vec<Record>> {
    ...
    let pool_record = get(pool_hash.clone(), GetOptions::default())?
        .ok_or(wasm_error!(WasmErrorInner::Guest("Pool not found".to_string())))?;
    if pool_record.action().author() != &caller {
        return Err(wasm_error!(WasmErrorInner::Guest(
            "Only the pool creator can view contributions".to_string()
        )));
    }
...
pub fn get_patient_pools(patient_hash: ActionHash) -> ExternResult<Vec<Record>> {
    ...
    let links = get_links(
        LinkQuery::try_new(patient_hash.clone(), LinkTypes::ContributorToPools)?,
        GetStrategy::default(),
    )?;
