...
pub fn get_patient_ips_documents(patient_hash: ActionHash) -> ExternResult<Vec<Record>> {
    ...
    let links = get_links(LinkQuery::try_new(patient_hash.clone(), LinkTypes::PatientToIps)?, GetStrategy::default())?;
...
pub fn finalize_ips(ips_hash: ActionHash) -> ExternResult<ActionHash> {
    ...
    let patient_hash = ips.patient_hash.clone();
    let auth = require_authorization(
        patient_hash.clone(),
        DataCategory::All,
        Permission::Amend,
    ...
    let updated_hash = update_entry(ips_hash, ips)?;
...
pub fn add_ips_allergy(input: AddAllergyInput) -> ExternResult<ActionHash> {
    let ips_record = get(input.ips_hash.clone(), GetOptions::default())?
        .ok_or(wasm_error!(WasmErrorInner::Guest("IPS not found".to_string())))?;

    let ips: InternationalPatientSummary = ips_record
        .entry()
        .to_app_option()
        .map_err(|e| wasm_error!(WasmErrorInner::Guest(e.to_string())))?
        .ok_or(wasm_error!(WasmErrorInner::Guest("Invalid IPS entry".to_string())))?;
...
pub fn get_ips_allergies(ips_hash: ActionHash) -> ExternResult<Vec<Record>> {
    let ips_record = get(ips_hash.clone(), GetOptions::default())?
        .ok_or(wasm_error!(WasmErrorInner::Guest("IPS not found".to_string())))?;
...
pub fn add_ips_immunization(input: AddImmunizationInput) -> ExternResult<ActionHash> {
    ...
    log_data_access(
        patient_hash,
        vec![DataCategory::Immunizations],
        Permission::Write,
        auth.consent_hash,
        auth.emergency_override,
        None,
