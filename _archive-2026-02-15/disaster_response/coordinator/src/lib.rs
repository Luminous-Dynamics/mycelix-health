...
pub fn triage_patient(input: TriagePatientInput) -> ExternResult<ActionHash> {
    ...
    let mut auth = None;
    if let Some(patient_hash) = input.patient_hash.clone() {
        auth = Some(require_authorization(
            patient_hash,
            DataCategory::All,
            Permission::Write,
            true,
    ...
    if let (Some(patient_hash), Some(auth)) = (input.patient_hash, auth) {
        log_data_access(
            patient_hash,
...
pub fn request_emergency_access(input: RequestEmergencyAccessInput) -> ExternResult<ActionHash> {
    ...
    let override_reason = input
        ...
        .clone()
        .or_else(|| Some(input.reason.clone()));
...
pub fn get_patient_emergency_access(patient_hash: ActionHash) -> ExternResult<Vec<Record>> {
    ...
    let links = get_links(
        LinkQuery::try_new(patient_hash.clone(), LinkTypes::PatientToEmergencyAccess)?,
        GetStrategy::default(),
    )?;
