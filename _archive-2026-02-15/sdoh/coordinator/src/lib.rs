...
pub fn get_screening(hash: ActionHash) -> ExternResult<Option<Record>> {
    ...
    let screening: SdohScreening = record
        .entry()
        .to_app_option()
        .map_err(|e| wasm_error!(WasmErrorInner::Guest(e.to_string())))?
        .ok_or(wasm_error!(WasmErrorInner::Guest("Invalid screening entry".to_string())))?;

    let patient_hash = screening.patient_hash.clone();
    let auth = require_authorization(patient_hash.clone(), DataCategory::All, Permission::Read, false)?;
...
pub fn create_intervention(input: CreateInterventionInput) -> ExternResult<Record> {
    // Ensure the screening belongs to the same patient
    let screening_record = get(input.screening_hash.clone(), GetOptions::default())?
        .ok_or(wasm_error!(WasmErrorInner::Guest("Screening not found".to_string())))?;
    let screening: SdohScreening = screening_record
        .entry()
        .to_app_option()
        .map_err(|e| wasm_error!(WasmErrorInner::Guest(e.to_string())))?
        .ok_or(wasm_error!(WasmErrorInner::Guest("Invalid screening entry".to_string())))?;
    if screening.patient_hash != input.patient_hash {
        return Err(wasm_error!(WasmErrorInner::Guest(
            "Screening patient does not match intervention patient".to_string()
        )));
    }
...
pub fn get_screening_interventions(screening_hash: ActionHash) -> ExternResult<Vec<Record>> {
    let screening_record = get(screening_hash.clone(), GetOptions::default())?
        .ok_or(wasm_error!(WasmErrorInner::Guest("Screening not found".to_string())))?;
    let screening: SdohScreening = screening_record
        .entry()
        .to_app_option()
...
pub fn update_intervention(input: UpdateInterventionInput) -> ExternResult<Record> {
    ...
    let patient_hash = intervention.patient_hash.clone();
    let auth = require_authorization(
        patient_hash.clone(),
        DataCategory::All,
        Permission::Amend,
...
pub fn create_follow_up(input: CreateFollowUpInput) -> ExternResult<Record> {
    let intervention_record = get(input.intervention_hash.clone(), GetOptions::default())?
        .ok_or(wasm_error!(WasmErrorInner::Guest("Intervention not found".to_string())))?;
    let intervention: SdohIntervention = intervention_record
        .entry()
        .to_app_option()
        .map_err(|e| wasm_error!(WasmErrorInner::Guest(e.to_string())))?
        .ok_or(wasm_error!(WasmErrorInner::Guest("Invalid intervention entry".to_string())))?;
...
pub fn get_intervention_follow_ups(intervention_hash: ActionHash) -> ExternResult<Vec<Record>> {
    let intervention_record = get(intervention_hash.clone(), GetOptions::default())?
        .ok_or(wasm_error!(WasmErrorInner::Guest("Intervention not found".to_string())))?;
    let intervention: SdohIntervention = intervention_record
        .entry()
        .to_app_option()
