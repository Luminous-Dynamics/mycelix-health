#![deny(unsafe_code)]
//! Exact-target qualification for one clinical causal assessment.
//!
//! Qualification composes two independent proofs:
//! - professional authority to clinically review this exact assessment;
//! - deployment admission of the exact observed event, exposure association, and
//!   causal evidence artifacts used by the assessment.
//!
//! A qualified result may still conclude `Indeterminate`. Qualification means the
//! assessment process/evidence lineage satisfied this contract, not that causality
//! was established.
...
impl CausalQualificationPolicyV1 {
    pub fn validate(&self) -> Result<(), CausalQualificationError> {
        if self.schema_version != 1 {
            return Err(CausalQualificationError::UnsupportedPolicyVersion(
                self.schema_version,
            ));
        }
...
/// Non-cloneable, non-serializable capability proving one exact causal assessment has
/// crossed both assessor-authority and evidence-trust boundaries.
...
impl QualifiedCausalAssessmentCapability {
    ...
    /// Consume the in-process capability into a serializable audit receipt. The
    /// resulting receipt is evidence for a later trusted runtime/DHT attestation; it
    /// does not recreate this capability when deserialized.
    pub fn into_audit_receipt(
        self,
    ) -> Result<QualifiedCausalAssessmentReceiptV1, CausalQualificationError> {
        let mut receipt = QualifiedCausalAssessmentReceiptV1 {
            schema_version: 1,
            ...
            receipt_digest: StoredDigest {
                ...
                domain: DigestDomain::ClinicalCausalQualifiedReceipt,
                value: [1u8; 32],
            },
        };
        let digest = receipt.compute_digest_without_self()?;
        receipt.receipt_digest = digest.stored();
        Ok(receipt)
    }
}
...
struct ReceiptDigestMaterial<'a> {
    ...
    assessor_jurisdiction: &'a JurisdictionCode,
    authority_policy_digest: StoredDigest,
    authority_evidence_digests: &'a [[u8; 32]],
    conclusion: CausalConclusionV1,
    qualified_at_micros: i64,
}
...
impl QualifiedCausalAssessmentReceiptV1 {
    pub fn validate_shape(&self) -> Result<(), CausalQualificationError> {
        if self.schema_version != 1 {
            return Err(CausalQualificationError::UnsupportedReceiptVersion(
                self.schema_version,
            ));
        }
        ...
        if self.compute_digest_without_self()?.stored() != self.receipt_digest {
            return Err(CausalQualificationError::ReceiptDigestMismatch);
        }
        Ok(())
    }

    fn compute_digest_without_self(&self) -> Result<VerifiedDigest, CausalQualificationError> {
...
pub fn qualify_causal_assessment(
    ...
    expected_evidence_trust_policy_digest: VerifiedDigest,
    authority: AuthorityPermit,
    expected_authority_policy_digest: VerifiedDigest,
    ...
    expected_evidence_trust_policy_digest
        .require_domain(DigestDomain::ClinicalCausalEvidenceTrustPolicy)?;
    expected_authority_policy_digest.require_domain(DigestDomain::AuthorityPolicy)?;
    ...
    if evidence_trust.trust_policy_digest() != expected_evidence_trust_policy_digest.stored() {
        return Err(CausalQualificationError::EvidenceTrustPolicyMismatch);
    }

    if authority.purpose() != AuthorityPurpose::ClinicalReview {
    ...
    if authority.policy_digest() != EvidenceDigest(expected_authority_policy_digest.value()) {
    ...
    Ok(QualifiedCausalAssessmentCapability {
        ...
        causal_evidence_trust_policy_digest: expected_evidence_trust_policy_digest.stored(),
        ...
        authority_policy_digest: expected_authority_policy_digest.stored(),
        authority_evidence_digests,
...
pub enum CausalQualificationError {
    ...
    #[error("causal evidence trust receipt uses the wrong deployment trust policy")]
    EvidenceTrustPolicyMismatch,
    #[error("causal assessment requires ClinicalReview authority")]
    WrongAuthorityPurpose,
    #[error("assessor authority targets a different causal assessment")]
    AuthorityTargetMismatch,
    #[error("assessor authority was evaluated under a different policy")]
