...
//!
//! This crate reduces already DHT-validated medication-dispense records. It does not
//! establish Holochain validity itself, and it never treats absence from a supplied
//! read set as proof of global absence.
//!
//! Core invariants:
//! - only finalized `MedicationDispenseReceipt` identities consume refill slots;
//! - duplicate publications of one exact semantic receipt are idempotent;
//! - different finalized receipts for one medication + activation + slot are conflict;
//! - allocator equivocation remains explicit in the read model;
//! - visible authorizations for different release outcomes/grantees are conflict;
//! - a later slot without its exact finalized predecessor in the supplied set fails closed;
//! - no last-write-wins rule converts conflict into a clean refill ledger.
...
pub struct FinalizedReceiptView {
    ...
    /// Multiple publication actions carrying the exact same semantic receipt are
    /// retained as provenance but count as one clinical dispense event.
...
pub enum DispenseSlotState {
    /// One exact semantic final receipt and no known allocator conflict.
    Finalized(FinalizedReceiptView),
    /// Slot authorization exists in the supplied set but no finalized dispense is
    /// present. This is evidence only; it is not a consumed refill slot and may have
    /// expired outside the supplied data.
    AuthorizationEvidenceOnly {
        authorization_action_hashes: Vec<ActionHash>,
    },
    /// Conflicting release authority and/or finalized outcomes are visible. Consumers
    /// must not choose one by timestamp or arrival order.
    Conflict {
...
pub struct DispenseLineageView {
    ...
    /// Highest contiguous, conflict-free finalized slot beginning at slot 0.
    /// `None` means no finalized slot has safely consumed the initial-fill slot.
...
///
/// This function rechecks exact cross-record lineage so a partial/mismatched read set
/// fails closed. It does not query the DHT and therefore does not turn local absence
/// into a claim of global uniqueness.
pub fn reduce_dispense_state(
    ...
    // Preserve authorization evidence even when physical/finalized release has not occurred.
    for (slot_key, records) in &authorizations_by_slot {
        let accumulator = slots.entry(*slot_key).or_default();
        for record in records {
            accumulator
                .authorization_action_hashes
                .push(record.action_hash.clone());
        }
    ...
    // Every non-initial authorization must close over the immediately preceding finalized
    // dispense in the supplied read set. This detects incomplete query materialization.
    for record in authorizations {
    ...
    for record in equivocation_evidence {
        ...
        if record.evidence.left_authorization_hash == record.evidence.right_authorization_hash {
            return Err(DispenseStateError::MalformedEquivocationEvidence);
        }
        ...
        if !release_authorizations_conflict(left, right) {
            // Protect the read model from an over-broad/incorrect equivocation entry.
    ...
    let mut by_lineage: BTreeMap<LineageKey, Vec<(u32, SlotAccumulator)>> = BTreeMap::new();
    ...
    for (lineage_key, mut lineage_slots) in by_lineage {
        ...
        validate_no_finalized_gaps(&views)?;
        let contiguous_finalized_through = contiguous_finalized_through(&views);
...
/// Authorization IDs and validity windows may differ during a retry. That is not
/// by itself equivocation if the same exact pharmacist/grantee is authorized to
/// publish the same exact final receipt. Different release result or grantee is.
...
fn validate_no_finalized_gaps(slots: &[DispenseSlotView]) -> Result<(), DispenseStateError> {
    let finalized_slots: Vec<u32> = slots
        .iter()
        .filter_map(|slot| match slot.state {
            DispenseSlotState::Finalized(_) => Some(slot.slot_index),
            DispenseSlotState::AuthorizationEvidenceOnly { .. }
            | DispenseSlotState::Conflict { .. } => None,
        })
        .collect();
...
fn contiguous_finalized_through(slots: &[DispenseSlotView]) -> Option<u32> {
...
fn require_activation_receipt_domain(digest: StoredDigest) -> Result<(), DispenseStateError> {
    digest.validate_shape().map_err(|_| DispenseStateError::InvalidDigest)?;
    if !matches!(
        digest.domain,
...
pub enum DispenseStateError {
    ...
    #[error("equivocation evidence references authorizations with the same release outcome/grantee")]
...
mod tests {
    ...
    use mycelix_clinical_integrity::{DigestAlgorithm, StoredDigest};
    ...
    fn authorization(
        ...
        SlotAuthorizationRecord {
            ...
            authorization: MedicationDispenseSlotAuthorization {
                ...
                valid_from: hdi::prelude::Timestamp::from_micros(10),
                valid_until: hdi::prelude::Timestamp::from_micros(100),
            },
        }
    }
    ...
    fn duplicate_publication_of_same_receipt_is_idempotent() {
        ...
        let slot = &view.lineages[0].slots[0];
        match &slot.state {
            DispenseSlotState::Finalized(finalized) => {
    ...
    fn same_final_receipt_and_grantee_retry_is_not_equivocation() {
        let mut auth_a = authorization(action(1), 0, 20, 30, None);
        let mut auth_b = authorization(action(2), 0, 20, 30, None);
        auth_b.authorization.authorization_id = "retry-id".into();
        auth_b.authorization.valid_from = hdi::prelude::Timestamp::from_micros(200);
        auth_b.authorization.valid_until = hdi::prelude::Timestamp::from_micros(250);
        assert!(!release_authorizations_conflict(
            &auth_a.authorization,
            &auth_b.authorization
        ));
        auth_a.authorization.grantee = agent(31);
        assert!(release_authorizations_conflict(
            &auth_a.authorization,
            &auth_b.authorization
        ));
    ...
    fn refill_requires_predecessor_in_supplied_read_set() {
        ...
        let error = reduce_dispense_state(&[auth], &[], &[]);
        assert!(matches!(
            error,
