#![deny(unsafe_code)]
//! High-assurance medication dispensing capability.
//!
//! V1 is intentionally narrow. It supports only an exact full-fill request against
//! exactly one current qualified/emergency medication activation. It does not infer
//! product substitution, partial-fill arithmetic, controlled-substance exceptions,
//! or global linearizability from a local DHT snapshot.
//!
//! The central separation-of-powers invariant is:
//!
//! `Prescribe` authority != `Dispense` authority.
//!
//! A pharmacist's authorship/identity is likewise not enough. Dispensing requires
//! exact professional authority, current activation provenance, verified pharmacy
...
const RECEIPT_SCHEMA_TAG: &[u8] = b"mycelix-health/medication-dispense-receipt-v1";
...
pub struct DispensePolicyV1 {
    ...
    /// Emergency-origin medication may be dispensed only when the deployment opts in.
...
/// Non-serializable evidence that the canonical activation view had exactly one
/// current lineage for this exact medication artifact when observed.
...
/// Evidence already resolved by a trusted pharmacy adapter. This establishes exact
/// evidence shape and temporal consistency, not institutional trust in the source.
...
pub struct MedicationDispenseRequestV1 {
    ...
    /// Zero-based fill slot. Slot 0 is the initial dispense; later slots correspond
    /// to `numberOfRepeatsAllowed` / authorized refills.
    pub slot_index: u32,
...
impl DispenseLedgerSnapshotV1 {
    ...
    pub fn next_slot(&self) -> Result<u32, DispenseError> {
...
/// Non-cloneable, non-serializable authorization for one exact full-fill dispense.
pub struct MedicationDispenseCapability {
    dispense_id: String,
...
#[derive(Clone, Debug, Serialize, Deserialize, PartialEq, Eq)]
#[serde(deny_unknown_fields)]
pub struct MedicationDispenseReceiptV1 {
    pub schema_version: u16,
    pub dispense_id: String,
...
impl MedicationDispenseReceiptV1 {
    pub fn validate_shape(&self) -> Result<(), DispenseError> {
        if self.schema_version != 1 {
            return Err(DispenseError::UnsupportedReceiptVersion(self.schema_version));
        }
...
#[allow(clippy::too_many_arguments)]
pub fn authorize_medication_dispense(
    medication: &MedicationRequestArtifact,
...
) -> Result<MedicationDispenseCapability, DispenseError> {
    ...
    if request.slot_index != ledger.next_slot()? {
    ...
    Ok(MedicationDispenseCapability {
...
pub fn consume_dispense_capability(
    capability: MedicationDispenseCapability,
) -> Result<(MedicationDispenseReceiptV1, VerifiedDigest), DispenseError> {
    let receipt = MedicationDispenseReceiptV1 {
        schema_version: 1,
        ...
        slot_index: capability.slot_index,
...
pub enum DispenseError {
    ...
    #[error("unsupported dispense receipt version {0}")]
    UnsupportedReceiptVersion(u16),
    #[error("dispense id is required")]
    MissingDispenseId,
    #[error("pharmacy id is required")]
    MissingPharmacyId,
    #[error("dispense receipt identity is incomplete")]
    ...
    #[error("duplicate prior dispense receipt in ledger snapshot")]
    ...
    #[error("requested fill slot does not equal the next ledger slot")]
...
mod tests {
    ...
    #[test]
    fn ledger_requires_contiguous_unique_slots() {
        ...
        let receipt0 = PriorDispenseEvidence::from_verified_receipt(
            0,
            verified(DigestDomain::MedicationDispenseReceipt, 3),
        )
        .unwrap();
        let receipt2 = PriorDispenseEvidence::from_verified_receipt(
            2,
    ...
    fn activation_receipt_domains_do_not_accept_dispense_receipt() {
        assert_eq!(
            require_activation_receipt_domain(stored(DigestDomain::MedicationDispenseReceipt, 1))
                .unwrap_err(),
            DispenseError::InvalidActivationReceiptDomain(
                DigestDomain::MedicationDispenseReceipt
            )
        );
    }
