#![forbid(unsafe_code)]
//! Product-facing QUAL-EVID-006 checkpoint adapter semantics.
//!
//! This crate closes the last naked current-head boolean in the first #208
//! Patient-v2 qualification vertical slice.  It consumes the exact typed
//! identity emitted by QUAL-EVID-004/#217 and cross-checks that identity against
//! the governed qualification, exact QUAL-EVID-002/#212 product profile, local
//! qualification-ledger admissibility, the expected QUAL-EVID-003/#214
//! profile-match commitment, and current consumption time.
//!
//! The lower QUAL-EVID-002 reference crate remains a dependency and still owns
//! its source-staged `VerifiedLineageHead` adapter type.  That type is NOT
//! re-exported here.  The private call into that reference is made only after the
//! stronger #217 artifact has passed every check in this crate.

use core::fmt;
use mycelix_qualification_adapter_profile_core as legacy;
...
///
/// There is deliberately no `verified` boolean in this object.  Construction
/// represents the adapter boundary from a strict #217 verified-head report into
/// Rust.  Signature/quorum/store verification stays #217's responsibility; this
/// crate independently validates that the complete authority identity matches
/// the #208 qualification/profile being consumed.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
...
/// Non-cloneable exact #208 authority token retaining #217 provenance.
...
///
/// Every attempted conversion observes time before semantic validation.  This
/// preserves QUAL-EVID-002's anti-time-rollback rule even for failed head
/// artifacts.
...
impl Profile208CompositionCommitmentGate {
    ...
    pub fn convert(
        ...
        // The raw boolean seam is confined to the lower non-product reference
        // dependency.  This crate never exports it and reaches it only after the
        // complete #217 authority identity above has passed.
...
mod tests {
    ...
    fn exact_qualified_head_emits_one_provenance_rich_token() {
        ...
        assert_eq!(
            token.checkpoint_bundle_digest(),
            CheckpointBundleDigest::new(bytes(93)).unwrap()
        );
        assert_eq!(
            token.verified_head_digest(),
            VerifiedHeadDigest::new(bytes(96)).unwrap()
        );
    }
    ...
    #[test]
    fn wrong_head_binding_receipt_policy_trust_and_deployment_are_rejected() {
        let qualification = verified(3, 10, 1_000);
        let ledger = admitted(&qualification);
        let expected = ProfileMatchCommitmentDigest::new(bytes(94)).unwrap();

        let wrong_subject = verified(4, 10, 1_000);
        let mut wrong_binding = head(&wrong_subject, 94, 150, 900);
        wrong_binding.head_receipt_digest = qualification.receipt_digest();
        wrong_binding.policy_digest = qualification.policy_digest();
        wrong_binding.trust_store_digest = qualification.trust_store_digest();
        wrong_binding.deployment_evidence = qualification.deployment_evidence().unwrap();
        let mut gate = Profile208CompositionCommitmentGate::new(profile(3, 500));
        assert_eq!(
            gate.convert(&qualification, &ledger, wrong_binding, expected, 200)
                .map(|_| ()),
            Err(AdapterConversionError::HeadBindingMismatch)
        );

        let mut wrong_receipt = head(&qualification, 94, 150, 900);
        wrong_receipt.head_receipt_digest = oid!(ReceiptDigest, 41);
        let mut gate = Profile208CompositionCommitmentGate::new(profile(3, 500));
        assert_eq!(
            gate.convert(&qualification, &ledger, wrong_receipt, expected, 200)
                .map(|_| ()),
    ...
    #[test]
    fn wrapper_public_surface_never_reexports_the_raw_boolean_head_type() {
        // The compile-time/public-API property is guarded in CI.  This unit test
        // anchors the exact #217 schema that replaced the boolean seam.
