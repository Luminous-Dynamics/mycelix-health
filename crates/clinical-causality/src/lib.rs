...
//!
//! This crate keeps observation, temporal association, and causal attribution separate.
//! A valid observed event can exist with unknown causality, and temporal proximity alone
//! can never mint a positive causal relationship conclusion.
...
impl TemporalRelationshipV1 {
    pub fn permits_positive_relationship(self) -> bool {
        !matches!(self, Self::EventBeforeExposure)
    }
}
...
impl ExposureAssociationV1 {
    pub fn derive(
        ...
        let observed_digest = observed.verified_digest()?;
        let exposure_end = exposure
            .exposure_end_micros
            .unwrap_or(exposure.exposure_start_micros);
        let temporal_relationship = if observed
            .end_micros
            .is_some_and(|end| end < exposure.exposure_start_micros)
            || (observed.end_micros.is_none() && observed.onset_micros < exposure.exposure_start_micros)
        {
            TemporalRelationshipV1::EventBeforeExposure
        } else if observed.onset_micros > exposure_end {
        ...
        let association = Self {
            schema_version: 1,
            observed_event_digest: observed_digest.stored(),
            exposure,
...
pub struct CausalEvidenceItemV1 {
    ...
    /// Exact evidence artifacts supporting this factor. These remain evidence claims
    /// until the owning adapter/runtime verifies the referenced artifacts.
...
impl CausalEvidenceItemV1 {
    fn validate(&self) -> Result<(), CausalityError> {
        ...
        {
            return Err(CausalityError::DirectionalEvidenceWithoutArtifact);
        }
        Ok(())
    }
}
...
impl ExternalCausalityScaleLabelV1 {
    fn validate(&self) -> Result<(), CausalityError> {
        if self.system.trim().is_empty() || self.version.trim().is_empty() || self.label.trim().is_empty()
        {
...
impl CausalAssessmentV1 {
    ...
    pub fn validate_against(
        ...
        if self.assessment_method.trim().is_empty() || self.assessment_method_version.trim().is_empty()
        {
        ...
        let supporting_non_temporal: BTreeSet<_> = self
            ...
            .filter(|item| {
                item.kind != CausalEvidenceKindV1::TemporalPlausibility
                    && matches!(item.direction, EvidenceDirectionV1::SupportsRelationship)
            })
            .map(|item| item.kind)
            .collect();
        let challenging_non_temporal: BTreeSet<_> = self
            ...
            .filter(|item| {
                item.kind != CausalEvidenceKindV1::TemporalPlausibility
                    && matches!(item.direction, EvidenceDirectionV1::ChallengesRelationship)
            })
            .map(|item| item.kind)
            .collect();
        ...
        match self.conclusion {
            ...
            CausalConclusionV1::TemporalAssociationOnly => {
                if !association.temporal_relationship.permits_positive_relationship() {
                    return Err(CausalityError::TemporalAssociationConclusionPredatesExposure);
                }
                if !supporting_non_temporal.is_empty() {
                    return Err(CausalityError::TemporalOnlyHasNonTemporalSupport);
                }
            }
    ...
    pub fn has_mixed_directional_evidence(&self) -> bool {
        let has_support = self.evidence.iter().any(|item| {
            item.direction == EvidenceDirectionV1::SupportsRelationship
        });
        let has_challenge = self.evidence.iter().any(|item| {
...
fn ensure_positive_temporality(association: &ExposureAssociationV1) -> Result<(), CausalityError> {
    if !association.temporal_relationship.permits_positive_relationship() {
        return Err(CausalityError::PositiveConclusionPredatesExposure);
    }
    Ok(())
}
...
pub enum CausalityError {
    ...
    #[error("supporting/challenging/mixed evidence requires at least one evidence artifact")]
    DirectionalEvidenceWithoutArtifact,
    ...
    #[error("temporal-association-only conclusion contains non-temporal supporting evidence")]
    TemporalOnlyHasNonTemporalSupport,
