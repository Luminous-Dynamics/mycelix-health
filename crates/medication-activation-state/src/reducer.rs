...
pub fn reduce_activation_state(
    ...
    Ok(MedicationActivationView {
        current,
        history,
        legacy_unqualified: collect_legacy_active(legacy)?,
    })
}
...
> {
    let mut groups: HashMap<StoredDigest, QualifiedGroup> = HashMap::new();
...
fn collect_emergency_activations(
    ...
    ActivationStateError,
> {
    let mut groups: HashMap<StoredDigest, EmergencyGroup> = HashMap::new();
    let mut action_to_receipt = HashMap::new();
...
fn map_qualified_reason(reason: &ActivationRevocationReason) -> QualifiedTerminationReason {
    match reason {
        ...
        ActivationRevocationReason::MedicationStopped => {
            QualifiedTerminationReason::MedicationStopped
        }
