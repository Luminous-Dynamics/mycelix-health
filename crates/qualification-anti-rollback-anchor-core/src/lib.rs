#![forbid(unsafe_code)]
//! Backend-neutral external anti-rollback reconciliation for qualification heads.
//!
//! QUAL-EVID-005 closes a specific gap: a locally self-verifying checkpoint store
//! cannot detect restoration of the *entire* local rollback domain to an older,
//! internally valid snapshot. This crate therefore requires an independently
//! verified external anchor before a #217 current head may cross the final #208
//! product-authority seam.
//!
//! This crate deliberately does not implement a transparency log, signature
//! verifier, HSM, TPM, or witness network. Those are backend responsibilities.
//! Backends cross this boundary through [`VerifiedExternalAnchorProof`], an
//! evidence-bearing trait with no caller-supplied `verified: bool`.
...
/// Privacy-minimal opaque state exported to an external rollback domain.
///
/// `verified_head_digest` is the #217 commitment to the full verified-head
/// identity. The external backend therefore does not need the underlying
/// qualification subject, receipt, policy, deployment, or care data.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct AnchorSubjectState {
...
impl LocalQualificationHead {
    pub fn from_verified_current_head(head: product::VerifiedCurrentHead208) -> Self {
        Self {
            state: AnchorSubjectState {
                lineage_binding_digest: head.lineage_binding_digest(),
                checkpoint_digest: head.checkpoint_digest(),
                checkpoint_epoch: head.checkpoint_epoch(),
                lineage_state_commitment: head.lineage_state_commitment(),
                verified_head_digest: head.verified_head_digest(),
            },
            head,
        }
    }

    pub fn state(&self) -> AnchorSubjectState {
        self.state
    }
...
/// Adapter contract implemented only by a backend that has already verified its
/// own signatures, witness quorum, trust state, monotonic history, and conflict
/// rules.
///
/// Implementing this trait is an authority boundary. A production implementation
/// must itself be qualified (for example by the generic transparency/witness core
/// tracked in Luminous-Dynamics/luminous-dynamics#1962). This crate never turns a
/// caller boolean into a proof.
pub trait VerifiedExternalAnchorProof {
    fn protocol_id(&self) -> &str;
    fn subject_domain(&self) -> &str;
    fn state(&self) -> AnchorSubjectState;
...
pub enum AnchorError {
    ...
    BootstrapRequiresCheckpointEpochOne,
...
/// Non-authorizing prepare record for one external anchor update.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
...
/// Prepare the first external anchor. V1 intentionally requires checkpoint epoch
/// one; onboarding a pre-existing later lineage requires an explicit migration /
/// bootstrap ceremony rather than silently blessing historical state.
pub fn prepare_anchor_bootstrap(
    local: &LocalQualificationHead,
    anchor_nonce: AnchorNonce,
    now_micros: i64,
) -> Result<AnchorUpdateIntent, AnchorError> {
    if local.state.checkpoint_epoch != 1 {
        return Err(AnchorError::BootstrapRequiresCheckpointEpochOne);
    }
...
/// Prepare an exact successor or same-state time-floor refresh.
///
/// Same checkpoint epoch + different state is a conflict. Same checkpoint epoch
/// + identical state may advance only the independently witnessed security-time
/// floor. A new checkpoint epoch must advance by exactly one.
pub fn prepare_anchor_successor<P: VerifiedExternalAnchorProof>(
    ...
    if local.state.lineage_binding_digest != prior.state().lineage_binding_digest {
    ...
    if local.state.checkpoint_epoch == prior_state.checkpoint_epoch {
        ...
        let expected = prior_state
            .checkpoint_epoch
            .checked_add(1)
            .ok_or(AnchorError::CheckpointEpochGap)?;
        if local.state.checkpoint_epoch != expected {
            return Err(AnchorError::CheckpointEpochGap);
        }
        if local.head.head_sequence() < prior_state_checkpoint_head_sequence(prior) {
...
/// The backend-neutral proof intentionally exposes only the opaque anchor state,
/// not the underlying #217 head sequence. V1 therefore does not infer receipt
/// sequence monotonicity from the external backend. #217/#219 remain responsible
/// for that theorem.
fn prior_state_checkpoint_head_sequence<P: VerifiedExternalAnchorProof>(_prior: &P) -> u64 {
    0
}

/// An exact externally anchored #217 head. Construction is private to successful
/// finalize/reconcile paths.
...
/// Finalize a prepare/commit cycle after the external backend returns a verified
/// proof for the exact intent. Until this succeeds the prepared local state has
/// no anchored authority.
...
/// Restart/recovery path. This never selects an arrival-order winner: only an
/// exact externally verified match can regain authority.
pub fn reconcile_existing_anchor<P: VerifiedExternalAnchorProof>(
    ...
    validate_external_proof(proof, now_micros)?;
    let anchored_state = proof.state();
    if local.state.lineage_binding_digest != anchored_state.lineage_binding_digest {
        return Err(AnchorError::SameEpochStateConflict);
    }
    if local.state.checkpoint_epoch < anchored_state.checkpoint_epoch {
        return Err(AnchorError::LocalRollbackDetected);
    }
    if local.state.checkpoint_epoch > anchored_state.checkpoint_epoch {
        return Err(AnchorError::ExternalAnchorBehind);
    }
    if local.state != anchored_state {
        return Err(AnchorError::SameEpochStateConflict);
    }
...
/// Final product token that retains both #219 evidence ancestry and independent
/// external-anchor provenance.
...
/// Stronger successor to #219's product gate. It will not consume a bare #217
/// head; external anchoring must already have succeeded.
...
mod tests {
    ...
    fn head(epoch: u64, checkpoint: u8, state: u8, verified: u8) -> product::VerifiedCurrentHead208 {
    ...
    #[test]
    fn bootstrap_requires_epoch_one_and_never_returns_authority() {
        let later = LocalQualificationHead::from_verified_current_head(head(2, 12, 15, 19));
        assert_eq!(
            prepare_anchor_bootstrap(&later, AnchorNonce::new(bytes(40)).unwrap(), 200),
            Err(AnchorError::BootstrapRequiresCheckpointEpochOne)
        );

        let first = LocalQualificationHead::from_verified_current_head(head(1, 12, 15, 19));
        let intent =
            prepare_anchor_bootstrap(&first, AnchorNonce::new(bytes(40)).unwrap(), 200).unwrap();
        assert_eq!(intent.anchor_sequence(), 1);
        assert_eq!(intent.previous_anchor_digest(), None);
        assert_eq!(intent.security_time_floor_micros(), 200);
    }

    #[test]
    fn exact_external_match_reconciles_after_restart() {
        let local = LocalQualificationHead::from_verified_current_head(head(1, 12, 15, 19));
    ...
    #[test]
    fn restoring_older_local_state_is_detected() {
        let older = LocalQualificationHead::from_verified_current_head(head(1, 12, 15, 19));
        let newer = LocalQualificationHead::from_verified_current_head(head(2, 22, 25, 29));
        let external = proof(&newer, 2, 250);
        assert_eq!(
            reconcile_existing_anchor(&older, &external, 300).map(|_| ()),
    ...
    fn external_anchor_behind_local_state_denies_authority() {
        let older = LocalQualificationHead::from_verified_current_head(head(1, 12, 15, 19));
        let newer = LocalQualificationHead::from_verified_current_head(head(2, 22, 25, 29));
        let external = proof(&older, 1, 200);
        assert_eq!(
            reconcile_existing_anchor(&newer, &external, 300).map(|_| ()),
            Err(AnchorError::ExternalAnchorBehind)
        );
    }

    #[test]
    fn same_epoch_different_commitment_is_conflict_not_winner_selection() {
        let a = LocalQualificationHead::from_verified_current_head(head(1, 12, 15, 19));
        let b = LocalQualificationHead::from_verified_current_head(head(1, 13, 15, 19));
    ...
    #[test]
    fn successor_requires_exact_epoch_increment_and_new_nonce() {
        let old = LocalQualificationHead::from_verified_current_head(head(1, 12, 15, 19));
        let prior = proof(&old, 1, 200);
        let skipped = LocalQualificationHead::from_verified_current_head(head(3, 32, 35, 39));
        assert_eq!(
        ...
        assert_eq!(
            prepare_anchor_successor(&old, &prior, prior.anchor_nonce(), 300),
            Err(AnchorError::ReusedAnchorNonce)
        );
    }

    #[test]
    fn same_state_can_refresh_only_the_security_time_floor() {
        let local = LocalQualificationHead::from_verified_current_head(head(1, 12, 15, 19));
    ...
    #[test]
    fn finalize_requires_exact_prepare_commit_identity() {
        let local = LocalQualificationHead::from_verified_current_head(head(1, 12, 15, 19));
        let intent =
        ...
        let anchored = finalize_anchor_update(&local, &intent, &external, 300).unwrap();
        assert_eq!(anchored.anchor_digest(), external.anchor_digest());
    ...
    #[test]
    fn expired_or_future_external_proof_never_authorizes() {
        let local = LocalQualificationHead::from_verified_current_head(head(1, 12, 15, 19));
