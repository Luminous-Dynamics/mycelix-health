...
//!
//! Inputs are assumed to be DHT-valid records supplied by a trusted adapter. This
//! crate does not query the DHT and cannot prove that the read set is globally
//! complete. It deliberately has no last-write-wins rule.
...
pub enum CausalAttestationLifecycle {
    /// Exactly one semantic public projection remains current.
    Current {
        projection: CausalProjectionView,
    },
    /// Multiple incompatible public projections remain for the same protected
    /// qualified-receipt commitment. No timestamp or insertion order selects a winner.
    ProjectionConflict {
        projections: Vec<CausalProjectionView>,
    },
    /// A root explicitly superseded the policy lineage. Historical evidence remains.
    Superseded {
        corrections: Vec<CausalCorrectionView>,
    },
    /// The semantic receipt lineage is no longer valid for current interpretation.
    Invalidated {
        corrections: Vec<CausalCorrectionView>,
    },
    /// An `Other` correction exists whose semantic effect is not represented by v1.
    ReviewRequired {
        corrections: Vec<CausalCorrectionView>,
    },
    /// Every known publication was explicitly classified as duplicate documentation.
...
/// corrections. Reference closure is enforced inside the supplied set, but the
/// reducer cannot prove that an omitted correction does not exist elsewhere.
pub fn reduce_causal_attestation_state(
    ...
    for (_, mut group) in groups {
        ...
        let lifecycle = if !unknown_effect.is_empty() {
            CausalAttestationLifecycle::ReviewRequired {
                corrections: unknown_effect,
            }
        } else if !invalidating.is_empty() {
...
mod tests {
    ...
    fn evidence_revocation_invalidates_entire_receipt_lineage() {
        let a = publication(1, 10, "a", CausalConclusionV1::EvidenceSuggestsRelationship, 20);
        let mut b = publication(2, 10, "b", CausalConclusionV1::EvidenceSuggestsRelationship, 20);
        b.attestation.attestation.qualification_policy_digest =
            a.attestation.attestation.qualification_policy_digest;
